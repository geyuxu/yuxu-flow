 ...
 * Run: GITHUB_TOKEN_CREATE_GIST=xxx deno run --allow-all blog/build-medium.ts
 */
...
const POSTS_JSON = join(__dirname, "posts.json");
const OUTPUT_DIR = join(__dirname, "medium");
const GIST_CACHE_FILE = join(__dirname, ".gist-cache.json");
const TABLE_CACHE_FILE = join(__dirname, ".table-cache.json");
...
renderer.table = function ({ header, rows }: Tokens.Table): string {
  ...
  if (tableCache[tableHash]) {
    const cachedPath = join(__dirname, tableCache[tableHash].replace("/blog/", ""));
...
async function build() {
  const posts: Post[] = JSON.parse(Deno.readTextFileSync(POSTS_JSON));
  ...
  for (const post of posts) {
    ...
    const mdPath = join(__dirname, `${post.slug}.md`);
  ...
  if (pendingTables.length > 0) {
    ...
    for (const table of pendingTables) {
      try {
        ...
        console.log(`  ✗ Failed to create table image: ${(err as Error).message}`);
      }
    }
  ...
  if (pendingGists.length > 0 && GITHUB_TOKEN_CREATE_GIST) {
    ...
    for (const gist of pendingGists) {
      try {
        ...
        console.log(`  ✗ Failed to create gist: ${(err as Error).message}`);
      }
    }
    ...
    console.log(
      `\n⚠ ${pendingGists.length} long code blocks found but no GITHUB_TOKEN_CREATE_GIST set.`
    );
    console.log(
      "  Run with: GITHUB_TOKEN_CREATE_GIST=xxx deno run --allow-all blog/build-medium.ts"
    );
  }
