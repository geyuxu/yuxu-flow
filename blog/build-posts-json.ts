...
const OUTPUT_FILE = join(__dirname, "posts.json");
...
function parseNotebook(content: string, filePath: string): NotebookParsed | null {
  try {
    ...
    console.log(
      `⚠ Error parsing notebook ${filePath}: ${(err as Error).message}`
    );
    return null;
  }
}
...
function getOrCreateMeta(
  ...
  if (existsSync(metaPath)) {
    ...
    try {
      ...
      console.log(`  ⚠ Error reading ${metaPath}: ${(err as Error).message}`);
      return null;
    }
  } else {
    ...
    try {
      ...
      console.log(`  ⚠ Error creating ${metaPath}: ${(err as Error).message}`);
      return null;
    }
  }
}
