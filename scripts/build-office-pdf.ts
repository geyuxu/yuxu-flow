...
async function main() {
  ...
  if (!soffice) {
    console.log("Warning: LibreOffice not found - skipping Office conversion");
  ...
  for (const filePath of officeFiles) {
    ...
    try {
      ...
      if (output.success) {
        console.log(`OK Converted: ${filename} -> ${basename(pdfPath)}`);
        converted++;
      } else {
        const stderr = new TextDecoder().decode(output.stderr);
        console.log(`FAIL converting ${filename}: ${stderr}`);
        failed++;
      }
    } catch (err) {
      console.log(`FAIL converting ${filename}: ${(err as Error).message}`);
      failed++;
    }
  }
